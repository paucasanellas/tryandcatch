...
3. **El PR de release no dispara el CI** (lo crea `GITHUB_TOKEN`): antes de mergearlo hay que cerrarlo y reabrirlo (`gh pr close <n> && gh pr reopen <n>`) para lanzar los checks obligatorios.
